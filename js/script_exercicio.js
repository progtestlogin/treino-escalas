...
const seqEscala = document.querySelector("#sobeArpejo");
...
async function playPause() {
    ...
    const escalaOitava = escala.map(note => Tonal.transpose(note, "8P"));
    const escala2Oitavas = escala.concat(escalaOitava).concat(Tonal.Note.transpose(escalaOitava[0], "8P"));
    ...
    if (seqNotas == "sobeArpejo") {
        
        for (let i = 0; i < 8; i++) {
        escalaPlay = escalaPlay.concat([ escala2Oitavas[(0+i)], escala2Oitavas[2+i], escala2Oitavas[4+i], escala2Oitavas[7+i] ]);
        }

    } else if (seqNotas == "desceArpejo") {

        for (let i = 7; i > -1; i--) {
            escalaPlay = escalaPlay.concat([ escala2Oitavas[(0+i)], escala2Oitavas[2+i], escala2Oitavas[4+i], escala2Oitavas[7+i] ]);
        }

    } else if (seqNotas == "sobeTerca") {

        for (let i = 0; i < 7; i++) {
        escalaPlay = escalaPlay.concat([ escala2Oitavas[(0+i)], escala2Oitavas[2+i] ]);
        }
        escalaPlay = escalaPlay.concat( escala2Oitavas[7] );
        escalaPlay = escalaPlay.concat( escala2Oitavas[7] );

    } else if (seqNotas == "desceTerca") {
        
        for (let i = 7; i > -1; i--) {
        escalaPlay = escalaPlay.concat([ escala2Oitavas[(0+i)], escala2Oitavas[2+i] ]);
        }
        
        escalaPlay = escalaPlay.concat( escala2Oitavas[0] );
        escalaPlay = escalaPlay.concat( escala2Oitavas[0] );
